...
function ct_obfuscate_shortcode($content) {
    if (!empty($content)) {
        $pattern = '/(\")(url|src|map_address|alt|background-image|oxycode|value)(\":\"[^\"]*)\[oxygen ([^\]]*)\]([^\"\[\s]*)/i';
...
function oxygen_activate_plugin() {
	...
	// setup preset defaults (may it is better to setup other defaults here as well)
	$defaults = apply_filters("oxygen_vsb_element_presets_defaults", array());
...
function condition_post_parent_id_callback($value, $operator) {
	...
	return oxy_condition_eval_int($current_post_parent_id, $value, $operator);
...
function condition_post_type_callback($value, $operator) {
	...
	return oxy_condition_eval_string($current_post_type, $value, $operator);
